...
## Running the XGBoost API in Docker

Docker is optional for local analysis. Use it when the trained model needs to
run as a repeatable prediction service.

First install the dependencies and train the model. This creates the model and
preprocessing artifacts under `artifacts/`:
...
```bash
docker build -t urban-happiness-xgb .
docker run --rm -p 8000:8000 urban-happiness-xgb
```

Check that it is running at <http://localhost:8000/health>. Interactive API
documentation is available at <http://localhost:8000/docs>. Send a prediction
to `/predict` with the six raw driver values:

```bash
curl -X POST http://localhost:8000/predict \
  -H "Content-Type: application/json" \
  -d '{"gdp_per_capita":30000,"healthy_life_expectancy":65,"pm2_5":20,"intentional_homicide_rate":5,"degree_of_urbanization":70,"sdg_index_score":75}'
```
